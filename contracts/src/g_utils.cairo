...
trait g_convertTrait {
    ...
    fn to(self: g_convert, other: g_convert) -> felt252;  // ← your original (safe for math)
    fn to_string(self: g_convert) -> ByteArray;
    
    // NEW: Promote self to the highest-precision type based on `other`
    fn auto(self: g_convert, other: g_convert) -> g_convert;
}
...
impl g_convertImpl of g_convertTrait {
    ...
    fn to(self: g_convert, other: g_convert) -> felt252 {
        match self {
            ...
            g_convert::U256(v) => match v.try_into() {
                ...
                Option::None => 0,  // fallback for large u256
            },
    ...
    fn auto(self: g_convert, other: g_convert) -> g_convert {
        // Check if either is U256 → promote to U256
        match (&self, &other) {
            (g_convert::U256(_), _) | (_, g_convert::U256(_)) => {
                g_convert::U256(self.g_convert::<u256>())
            },
            (g_convert::U128(_), _) | (_, g_convert::U128(_)) => {
                g_convert::U128(self.g_convert::<u128>())
            },
            (g_convert::U64(_), _) | (_, g_convert::U64(_)) => {
                g_convert::U64(self.g_convert::<u64>())
            },
            (g_convert::U32(_), _) | (_, g_convert::U32(_)) => {
                g_convert::U32(self.g_convert::<u32>())
            },
            (g_convert::U16(_), _) | (_, g_convert::U16(_)) => {
                g_convert::U16(self.g_convert::<u16>())
            },
            (g_convert::U8(_), _) | (_, g_convert::U8(_)) => {
                g_convert::U8(self.g_convert::<u8>())
            },
            _ => {
                // Default: promote to felt252 (covers Felt252, Bool, etc.)
                g_convert::Felt252(self.g_convert::<felt252>())
            }
        }
    }
}
...
// ... (keep all your existing TryInto impls exactly as they are) ...
// [All your existing TryInto implementations go here unchanged]
