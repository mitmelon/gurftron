...
mod GurftronDB {
    ...
    impl DatabaseImpl of IDatabase<ContractState> {
        ...
        fn insert(
            ...
            let profile = self.user_profiles.entry(caller).read();
            if !self.is_user_premium.entry(caller).read() {
    ...
    fn reward_active_validators(ref self: ContractState, validators: Array<ContractAddress>, bonus_points: u32) {
        ...
        while i < validators.len() {
            ...
            let profile = self.user_profiles.entry(validator).read();
            if profile.total_votes_cast > 0 {
                let current_points = self.points.entry(validator).read();
                let new_points = current_points + bonus_points.try_into().unwrap();
                self.points.entry(validator).write(new_points);
